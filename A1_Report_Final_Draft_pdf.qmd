  ...
  pdf:
    ...
    include-in-header:
      text: |
        ...
        \usepackage{booktabs} % nicer table rules for kable/latex
        \usepackage{caption}
        %\captionsetup[table]{position=top} % caption above tables
filters: 
  - clean-unicode.lua
editor: visual
engine: knitr
...
#builds K-1 binary labels for CORN. for each threshold k, target is 1 if y>k else 0
...
(ii) imbalance and shift: the test window contains no "High" days and is dominated by Levels 0-1. In that regime, copying yesterday is genuinely hard to beat. The LSTM-CORN learns the ordinal structure and avoids wild swings, but with few upper-level examples and a short 10-day context, it struggles to escalate to 2-3 when the series does move. Persistence is not affected by the data imbalance like the LSTM, and is in fact aided by it (response is more uniform for test set).
