...
We examined whether OS grids denote unique points. Longitude and latitude vary within OS grids, meaning that multiple longitude and latitude pairs had the same OS grids, indicating that an OS grid represents an area rather than a single coordinate.
...
We first measured missingness per variable and inspected its pattern. Where the absence itself could be informative, we created explicit 0/1 "was-missing" flags (before any outlier recoding). The idea is that if conditions were severe enough to prevent field teams from collecting measurements, then the missingness itself carries information, and this signal should be retained in the dataset.

These indicators were made for: AV.cat, Ski.pen, Crystals, Wetness, Snow.Index, and the summit weather fields (Summit.Wind.Dir, Summit.Wind.Speed, Summit.Air.Temp). Doing this early ensures the indicators reflect the data as collected and aren't confounded by later plausibility filters.
...
-   Snow temperature ($^\circ$C): values $> 5^\circ$C set to NA (snow cannot persist above $\sim 0^\circ$C; a small buffer accommodates sensor and entry noise).\
...
-   Maximum temperature gradient (assumed to be measured as $^\circ$C/10 cm): values $> 10$ set to NA.\
...
#### Altitude repair via coordinate-based altitude lookup

For the small number of missing altitudes, we queried an open altitude service at rounded (`latitude`, `longitude`) pairs and filled `Alt` where a value was returned. One queried coordinate yielded $0$ m (a sea-loch), indicating a geolocation error. For that single case, we replaced the coordinate with the area mean (Creag Meagaidh) and re-queried. To ensure deterministic compilation, altitude results are cached locally; subsequent runs read from cache if the service is unavailable.
...
Because angles wrap around at $360^\circ$, treating them as ordinary numbers creates an artificial jump between values such as $359^\circ$ and $0^\circ$. We therefore replaced each directional variable (`Wind.Dir`, `Summit.Wind.Dir`, `Aspect`) with two new variables that locate the direction on the unit circle: the sine and cosine of the angle (in radians). This keeps directions like $0^\circ$ and $360^\circ$ close in feature space, and removes the wrap-around discontinuity, which yields smoother relationships for the models. Missing angles remained missing in both components and were imputed later.
...
Since the dataset is comprised of time-series data, it was split chronologically into $80\%$ training and $20\%$ test by calendar time (no overlap). Any rows missing a `Date` or the target (`FAH_ord`) were removed before the split. A clear class imbalanced was noticed within the train and test split; the High level is very rare and does not appear in the test window.

For dealing with missing values, all preprocessing steps were fit on the training set only (e.g., imputation, scaling, encoding) and then applied unchanged to the test set. This setup avoids information leakage.
...
2.  **Rare-level consolidation:** Collapsed very rare categorical levels to `"other"` using $\texttt{threshold} = 0.005$ to avoid sparse dummies when creating one-hot encoded variables.\
...
4.  **Encoding:** One-hot encoding of categorical predictors (including `Area`) was used.\
5.  **Variance filtering:** Predictors with zero-variance and near-zero-variance were removed since they would add little to none predictive power.\
6.  **Scaling:** All numerical predictors were standardised, excluding the 0/1 informative-missing indicators (and area dummies) to preserve their interpretability.\
7.  **Seasonality:** The discrete season dummies were replaced with smooth cyclical features $doy\_sin$ and $doy\_cos$ (constructed from day-of-year with leap years handled); season binaries were removed to reduce collinearity.\
8.  **Target mapping:** The ordered response (`FAH_ord)` was mapped to integers $0{-}4$ to provide a single source of truth for neural-network models.

The recipe was prepped on the training set (estimating imputation models, encoding maps, and scaling parameters) and then applied to both training and test splits to yield model-ready matrices (`x_train`, `x_test`). For downstream modelling and plotting, a `Date` column was retained; final matrices ($X\_{train}$, $X\_{test}$) include the integer-encoded target as the last column.
...
We fixed random seeds and kept all data-prep and modelling steps in a single scripted pipeline so that results are reproducible across runs. After applying (i.e. baking) the recipe, there were no remaining missing values in either split. We re-checked the composite key (`Date`, `OSgrid`, `Area`) after duplicate handling, and we spot-checked distributions of imputed variables to make sure they looked reasonable (plots not shown for space). Altitude queries are cached locally, so reruns do not depend on the external service.
