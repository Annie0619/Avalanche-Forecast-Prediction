...
{\Large Andomei Smit (SMTAND051) \par}
...
6.  **Scaling:** Standardisation of numerical predictors, excluding the 0/1 informative-missing indicators (and area dummies).\
...
with open("artifacts/test_metrics.json") as f:
    m = json.load(f)
model_row = {"Model": "LSTM-CORN (tuned tau)",
...
```{python}
#| label: tbl:class-balance-html
#| results: asis
#| echo: false

import pandas as pd

# Build a wide table with a MultiIndex header to get grouped HTML headers
wide_html = (
    class_balance.assign(pct=lambda d: d["pct"].round(1))
        .pivot(index="FAH", columns="split", values=["count", "pct"])
        .sort_index()
...
# Ensure expected columns exist and in the intended order
needed = [("count","Test"), ("count","Train"), ("pct","Test"), ("pct","Train")]
for col in needed:
    if col not in wide_html.columns:
        wide_html[col] = 0
wide_html = wide_html[needed].reset_index().rename(columns={"FAH": "FAH level"})

# Rename level names for prettier group headers
wide_html.columns = pd.MultiIndex.from_tuples(
    [("","FAH level")] + [("count","Test"), ("count","Train"), ("percent","Test"), ("percent","Train")],
    names=["", ""]
)

# Styler: bootstrap-y look, caption on top, alignment tweaks
styler = (
    wide_html.style
        .hide(axis="index")  # use .hide_index() if your pandas is older
        .set_caption("FAH class balance by split.")
        .set_table_attributes('class="table table-sm table-striped"')
        .format({
            ("count","Test"):  "{:,.0f}",
            ("count","Train"): "{:,.0f}",
            ("percent","Test"):  "{:.1f}%",
        ...
        .set_properties(subset=[("","FAH level")], **{"text-align": "left", "white-space": "nowrap"})
        .set_table_styles([
...
print(styler.to_html(escape=False))
```
...
```{python}
#| label: tbl-metrics-html
#| results: asis
import pandas as pd

# Start from the existing `tbl` built above
pretty = (
...
# Style for Quarto HTML (Bootstrap 5)
styled = (
    pretty.style
        .hide(axis="index")  # if this errors on older pandas, use: .hide_index()
        .set_table_attributes('class="table table-sm table-striped"')
        .set_caption("Performance comparison between LSTM-CORN and baseline models.")
...
# Emit raw HTML so Quarto can render it
print(styled.to_html(escape=False))
```
...
styler = (
    pretty.style
      .hide(axis="index")  # if this errors on older pandas, use .hide_index()
      .set_caption("Test-set performance: model vs baselines.")
      .set_table_attributes('class="table table-sm table-striped"')
      .set_properties(subset=["Model"], **{"text-align":"left", "white-space":"nowrap"})
      .set_table_styles([
          {"selector":"th", "props":[("text-align","right")]},
...
# 
# # Load labels if not already defined
# try:
#     _ = (y_true, y_pred)
# except NameError:
#     z = np.load("artifacts/test_preds_ytrue.npz")
...
# 
# # 1) drop rows with NaN/inf in either vector
# def _finite_mask(a):
#     if a.dtype.kind == "f":
#         return np.isfinite(a)
#     return np.ones(a.shape, dtype=bool)
# 
# mask = _finite_mask(yt) & _finite_mask(yp)
# yt, yp = yt[mask], yp[mask]
# 
# # 2) sanitize labels
# def sanitize_two(ya, yb, tol=1e-9):
#     """Return integer label arrays; if not integer-like, label-encode."""
#     def _is_intlike(x):
#         return x.dtype.kind == "i" or (
#             x.dtype.kind == "f" and np.all(np.isclose(x, np.rint(x), atol=tol))
#         )
# 
#     if ya.dtype.kind in "OUSb" or yb.dtype.kind in "OUSb":
#         # strings/objects/bools -> encode together
#         enc = LabelEncoder()
#         enc.fit(np.concatenate([ya.astype(str), yb.astype(str)]))
#         return enc.transform(ya.astype(str)), enc.transform(yb.astype(str)), enc.classes_.astype(str)
# 
#     if _is_intlike(ya) and _is_intlike(yb):
#         return np.rint(ya).astype(np.int64), np.rint(yb).astype(np.int64), None
# 
#     # mixed / real-valued: fall back to encoding their string forms
#     enc = LabelEncoder()
#     enc.fit(np.concatenate([ya.astype(str), yb.astype(str)]))
#     return enc.transform(ya.astype(str)), enc.transform(yb.astype(str)), enc.classes_.astype(str)
# 
# yt_i, yp_i, class_names = sanitize_two(yt, yp)
# 
# # 3) choose labels to report
# labels_list = np.unique(np.concatenate([yt_i, yp_i]))
# # If you specifically want classes 0..4 *and* all values are within that set, keep it:
# allowed = np.array([0,1,2,3,4], dtype=np.int64)
# if np.isin(yt_i, allowed).all() and np.isin(yp_i, allowed).all():
#     labels_list = allowed
# 
# # 4) metrics
# rep = classification_report(
#     yt_i, yp_i,
#     labels=labels_list,
#     target_names=(class_names[labels_list] if class_names is not None and labels_list.max() < len(class_names) else None),
#     output_dict=True,
#     zero_division=0
# )
# 
# order = [str(k) for k in labels_list] + ["accuracy", "macro avg", "weighted avg"]
# df = (pd.DataFrame(rep).T
#         .reindex(order)
#         .reset_index()
#         .rename(columns={"index":"Class"}))
...
# df = df[["Class","precision","recall","f1-score","support"]].copy()
# 
# # Fill scalar accuracy into row if needed
# if "accuracy" in df["Class"].values:
#     for col in ["precision","recall","f1-score"]:
#         if pd.isna(df.loc[df["Class"]=="accuracy", col]).all():
#             df.loc[df["Class"]=="accuracy", col] = float(rep.get("accuracy", np.nan))
# 
# df[["precision","recall","f1-score"]] = df[["precision","recall","f1-score"]].round(3)
# df["support"] = df["support"].astype("Int64")  # nullable int
# 
# # Style and emit HTML
# styler = (
#     df.style
#       .hide(axis="index")  # use .hide_index() on pandas<2
#       .set_caption("Per-class precision, recall, F1 and support on the test set.")
#       .set_table_attributes(`class="table table-sm table-striped"`)
#       .format({"precision":"{:.3f}","recall":"{:.3f}","f1-score":"{:.3f}","support":"{:,.0f}"})
#       .set_properties(subset=["Class"], **{"text-align":"left","white-space":"nowrap"})
#       .set_table_styles([
#           {"selector":"th","props":[("text-align","right")]},
#           {"selector":"td","props":[("text-align","right")]},
#           {"selector":"caption","props":[("caption-side","top"),("font-weight","600")]}
...
try:
    _ = comp  # ensure comp exists
except NameError:
    print('<div class="alert alert-warning" role="alert">Note: <code>comp</code> is not defined (model_full/dl_te/taus may be missing). Run the prediction cell first.</div>')
else:
    pretty = comp.rename(columns={"acc": "Accuracy", "macroF1": "Macro-F1"}).copy()
    # numeric copies for comparisons
    _num = pretty[["Accuracy","Macro-F1","MAE","QWK"]].copy()
    _num["Accuracy"] = comp["acc"].astype(float)
    _num["Macro-F1"] = comp["macroF1"].astype(float)
    _num["MAE"]      = comp["MAE"].astype(float)
    _num["QWK"]      = comp["QWK"].astype(float)

    # string formatting for display
    pretty["Accuracy"] = _num["Accuracy"].map("{:.3f}".format)
    pretty["Macro-F1"] = _num["Macro-F1"].map("{:.3f}".format)
    pretty["MAE"]      = _num["MAE"].map("{:.3f}".format)
    pretty["QWK"]      = _num["QWK"].map("{:.3f}".format)

    # build styler
    styler = (
        pretty.style
          .hide(axis="index")  # if this errors on older pandas, use .hide_index()
          .set_caption("Performance comparison between tuned $\\tau$ cutoffs and a flat 0.5 cutoff.")
          .set_table_attributes('class="table table-sm table-striped"')
          .set_properties(subset=["Cutoffs"], **{"text-align":"left", "white-space":"nowrap"})
          .set_table_styles([
              {"selector":"th", "props":[("text-align","right")]},
              {"selector":"td", "props":[("text-align","right")]},
              {"selector":"caption", "props":[("caption-side","top"), ("font-weight","600")]}
    ...
    # optional: bold the best values (higher is better except MAE)
    best_vals = {
        "Accuracy": float(_num["Accuracy"].max()),
        "Macro-F1": float(_num["Macro-F1"].max()),
        "QWK":      float(_num["QWK"].max()),
        "MAE":      float(_num["MAE"].min()),
    }

    def _hi_best(col, higher_is_better=True):
        arr = _num[col].to_numpy(float)
        target = arr.max() if higher_is_better else arr.min()
        return ['font-weight:700' if v == target else '' for v in arr]

    styler = styler.apply(lambda s: _hi_best("Accuracy", True), subset=["Accuracy"])
    styler = styler.apply(lambda s: _hi_best("Macro-F1", True), subset=["Macro-F1"])
    styler = styler.apply(lambda s: _hi_best("QWK", True), subset=["QWK"])
    styler = styler.apply(lambda s: _hi_best("MAE", False), subset=["MAE"])
