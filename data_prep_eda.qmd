...
The above values seem to be duplicates, although some rows are almost exactly the same, except for some having an NA, whereas another row has a value. Begin by investigating the conflicts:

```{r}
# define the keys that identify duplicates
...
numeric_conflicts <- aval %>%
  ...
  # flag only the groups where at least one numeric column has >1 distinct value
  filter(if_any(ends_with("_nunique"), ~ .x > 1))

numeric_conflicts # there are some that have distinct values in the numeric variables
```
...
# sanity: which keys are missing?
missing_keys <- setdiff(key_cols, names(aval))
if (length(missing_keys)) {
  stop("These key columns are not in `aval`: ", paste(missing_keys, collapse = ", "))
}

# build list of NON-key columns explicitly
non_key_cols <- setdiff(names(aval), key_cols)

# helper: first non-NA while preserving type
first_non_na <- function(x) {
  nx <- x[!is.na(x)]
  if (length(nx) == 0) x[NA_integer_] else nx[1]
}

# 1) identify groups with numeric conflicts
numeric_conflicts <- aval %>%
  group_by(across(all_of(key_cols))) %>%
  summarise(
    across(where(is.numeric), ~ n_distinct(na.omit(.x)), .names = "{.col}_nuniq"),
    .groups = "drop"
  ) %>%
  filter(if_any(ends_with("_nuniq"), ~ .x > 1)) %>%
  select(all_of(key_cols)) %>%
  distinct()

# 2) collapse only groups with NO numeric conflicts
collapsed_ok <- aval %>%
  anti_join(numeric_conflicts, by = key_cols) %>%
  group_by(across(all_of(key_cols))) %>%
  summarise(
...
# 3) keep conflicting groups as-is (flag them)
kept_conflicts <- aval %>%
  semi_join(numeric_conflicts, by = key_cols) %>%
...
# 5) make this aval
aval <- aval_dups_resolved %>%
  select(-.collapsed, -.rows_collapsed)
```
...
aval <- aval %>%
  mutate(
    av_cat_missing            = as.factor(is.na(AV.Cat)),
    ski_pen_missing           = as.factor(is.na(Ski.Pen)),
    crystals_missing          = as.factor(is.na(Crystals)),
    wetness_missing           = as.factor(is.na(Wetness)),
    snow_index_missing        = as.factor(is.na(Snow.Index)),
    summit_wind_dir_missing   = as.factor(is.na(Summit.Wind.Dir)),
    summit_wind_speed_missing = as.factor(is.na(Summit.Wind.Speed)),
    summit_air_temp_missing   = as.factor(is.na(Summit.Air.Temp))
  )
```
...
# also fix the corresponding sin and cos Aspects:
# Recalculate sine and cosine encodings after cleaning Aspect
aval <- aval %>%
  mutate(
